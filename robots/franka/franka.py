...
class Franka(Robot):
    ...
    def __init__(self, config: FrankaConfig):
        ...
        config_camera = config.cameras
        config_camera.pop("left_wrist_image", None)
        print(config_camera)
        self.cameras = make_cameras_from_configs(config_camera)
    ...
    def _check_franka_connection(self, robot_ip: str):
        try:
            ...
            franka = FrankaInterfaceClient(ip=robot_ip, port=self.config.robot_port)
    ...
    def reset(self) -> None:
        ...
        self._robot.robot_move_to_ee_pose(pose=ee_positions_reset, time_to_go=5.0)
    ...
    def action_features(self) -> dict[str, type]:
        ...
        if self.config.control_mode == "isoteleop":
            ...
            features = {}
            # Delta EE pose (always present) - 使用 right_ 前缀适配统一双臂格式
    ...
    def _send_action_cartesian(self, action: dict[str, Any]) -> None:
        ...
        # Check for restart request (A button)
        if action.get("restart_requested", False):
            logger.info("[ROBOT] Restart requested, restarting joint impedance controller...")
            try:
                self._robot.robot_start_joint_impedance_control()
                logger.info("[ROBOT] Controller restarted successfully")
            except Exception as e:
                logger.error(f"[ROBOT] Failed to restart controller: {e}")
            return
        ...
        if action.get("reset_requested", False):
            ...
            try:
                ...
                self._robot.robot_start_joint_impedance_control()
                logger.info("[ROBOT] Reset completed, controller restarted")
            except Exception as e:
        ...
        delta_ee_pose = np.array([action[f"right_delta_ee_pose.{axis}"] for axis in ["x", "y", "z", "rx", "ry", "rz"]])
        ...
        if not self.config.debug:
            ...
            try:
                ...
                if "right_gripper_cmd_bin" in action:
                    self._handle_gripper(action["right_gripper_cmd_bin"], is_binary=True)
                return
            ...
            if num_steps > 1:
                ...
                for step in range(1, num_steps + 1):
                    ...
                    try:
                        ...
                        logger.warning(f"[ROBOT] robot_update_desired_ee_pose failed during interpolation: {e}")
                        # 控制器可能已停止，尝试重启
                        logger.info("[ROBOT] Attempting to restart controller...")
                        try:
                            self._robot.robot_start_joint_impedance_control()
                            logger.info("[ROBOT] Controller restarted successfully")
                        except Exception as e2:
                ...
                try:
                    ...
                    error_msg = str(e)
                    logger.warning(f"[ROBOT] robot_update_desired_ee_pose failed: {e}")
                    # 控制器可能已停止，尝试重启
                    logger.info("[ROBOT] Attempting to restart controller...")
                    try:
                        self._robot.robot_start_joint_impedance_control()
    ...
    def get_observation(self) -> dict[str, Any]:
        ...
        if self.config.use_gripper:
            ...
            obs_dict["right_gripper_cmd_bin"] = self._last_gripper_position
        else:
