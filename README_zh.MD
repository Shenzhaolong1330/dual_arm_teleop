...
## 4. 数据集回放
```bash
dual-arm-replay #注意cfg配置
```

## 5. 数据集可视化
```bash
dual-arm-visualize #注意cfg配置
```
<p align="center">
...
4. 运行命令 `dual-arm-record`，为确保安全， 请保证主臂与从臂关节角近似，`dual-arm-record` 会自动执行 `2.4 获取主臂-从臂关节角误差周期` 检查，然后按照 `8. 录制控制按键说明` 完成录制操作。
